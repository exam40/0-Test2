...
const TUTORIAL_STEPS = [
  ...
  { icon: '🎨', title: 'Track Your Progress', desc: 'Mark each paper as Not Started → Learning → Practised → Ready. Add topics to track per-subject progress with visual bars.', btn: 'Done! 🚀' }
];
...
// ===== RESULTS: SEAB O-LEVEL PAPER METADATA =====
// Weightages and tested components are compiled from the SEAB O-Level syllabi (PDFs at seab.gov.sg).
// 'weightage' is each paper's share of the subject; 'maxMarks' caps the score slider.
// Subjects sharing a `group` key are graded together (e.g. Combined Humanities = Social Studies + elective).
const SEAB_SUBJECT_META = {
  'English Language': {
    papers: {
      'Paper 1': { maxMarks: 68, weightage: 30, tests: ['Editing', 'Situational Writing', 'Continuous Writing'] },
      'Paper 2': { maxMarks: 70, weightage: 50, tests: ['Visual Text', 'Reading Comprehension', 'Summary'] },
      'LC': { maxMarks: 30, weightage: 10, tests: ['Listening Comprehension'] },
      'Oral': { maxMarks: 30, weightage: 10, tests: ['Reading Aloud', 'Spoken Interaction'] }
    }
  },
  'Mathematics': {
    papers: {
      'Paper 1': { maxMarks: 80, weightage: 50, tests: ['Number & Algebra', 'Geometry & Measurement', 'Statistics & Probability'] },
      'Paper 2': { maxMarks: 100, weightage: 50, tests: ['Number & Algebra', 'Geometry & Measurement', 'Statistics & Probability'] }
    }
  },
  'Additional Mathematics': {
    papers: {
      'Paper 1': { maxMarks: 80, weightage: 50, tests: ['Pure Mathematics (no calculator)'] },
      'Paper 2': { maxMarks: 100, weightage: 50, tests: ['Pure Mathematics', 'Calculus (with calculator)'] }
    }
  },
  'Chemistry': {
    papers: {
      'Paper 1': { maxMarks: 30, weightage: 30, tests: ['MCQ — entire syllabus'] },
      'Paper 2': { maxMarks: 80, weightage: 50, tests: ['Structured & Free Response'] },
      'Practical': { maxMarks: 30, weightage: 20, tests: ['School-based Practical Assessment'] }
    }
  },
  'Physics': {
    papers: {
  ...
  'Science (Phy/Chem)': {
    papers: {
      'Paper 1': { maxMarks: 40, weightage: 30, tests: ['MCQ — Physics & Chemistry'] },
      'Paper 2': { maxMarks: 50, weightage: 30, tests: ['Physics Structured & Free Response'] },
      'Paper 3': { maxMarks: 50, weightage: 30, tests: ['Chemistry Structured & Free Response'] },
      'Practical': { maxMarks: 40, weightage: 10, tests: ['Physics & Chemistry Practical'] }
    }
  },
  'Science (Phy/Bio)': {
    papers: {
      'Paper 1': { maxMarks: 40, weightage: 30, tests: ['MCQ — Physics & Biology'] },
      'Paper 2': { maxMarks: 50, weightage: 30, tests: ['Physics Structured & Free Response'] },
      'Paper 4': { maxMarks: 50, weightage: 30, tests: ['Biology Structured & Free Response'] },
      'Practical': { maxMarks: 40, weightage: 10, tests: ['Physics & Biology Practical'] }
    }
  },
  'Science (Chem/Bio)': {
    papers: {
      'Paper 1': { maxMarks: 40, weightage: 30, tests: ['MCQ — Chemistry & Biology'] },
      'Paper 3': { maxMarks: 50, weightage: 30, tests: ['Chemistry Structured & Free Response'] },
      'Paper 4': { maxMarks: 50, weightage: 30, tests: ['Biology Structured & Free Response'] },
      'Practical': { maxMarks: 40, weightage: 10, tests: ['Chemistry & Biology Practical'] }
    }
  },
  'Geography': {
    papers: {
      'Paper 1': { maxMarks: 100, weightage: 50, tests: ['Physical & Human Geography'] },
      'Paper 2': { maxMarks: 100, weightage: 50, tests: ['Geographical Skills & Investigations'] }
    }
  },
  'History': {
    papers: {
      'Paper 1': { maxMarks: 100, weightage: 50, tests: ['Source-Based Case Study', 'Structured Essay (Units 1–2)'] },
      'Paper 2': { maxMarks: 100, weightage: 50, tests: ['Source-Based Case Study', 'Structured Essay (Units 3–4)'] }
    }
  },
  'Literature in English': {
    papers: {
      'Paper 1': { maxMarks: 100, weightage: 50, tests: ['Prose & Poetry (set texts)'] },
      'Paper 2': { maxMarks: 100, weightage: 50, tests: ['Drama', 'Unseen'] }
    }
  },
  'Social Studies': {
    group: 'combined-humanities',
    papers: {
      'Paper 1': { maxMarks: 50, weightage: 50, tests: ['Source-Based Case Study', 'Structured Response Questions'] }
    }
  },
  'Humanities (SS, Geog)': {
    group: 'combined-humanities', label: 'Humanities (SS, Geog)',
    papers: {
      'Paper 2': { maxMarks: 50, weightage: 50, tests: ['Geography Elective'] }
    }
  },
  'Humanities (SS, Hist)': {
    group: 'combined-humanities', label: 'Humanities (SS, Hist)',
    papers: {
      'Paper 2': { maxMarks: 50, weightage: 50, tests: ['History Elective'] }
    }
  },
  'Humanities (SS, Lit)': {
    group: 'combined-humanities', label: 'Humanities (SS, Lit)',
    papers: {
      'Paper 2': { maxMarks: 50, weightage: 50, tests: ['Literature Elective'] }
    }
  },
  'Chinese': {
    papers: {
      'Paper 1': { maxMarks: 60, weightage: 30, tests: ['Writing (Essay / Functional)'] },
      'Paper 2': { maxMarks: 70, weightage: 35, tests: ['Language Usage & Reading Comprehension'] },
      'LC': { maxMarks: 20, weightage: 10, tests: ['Listening Comprehension'] },
      'Oral': { maxMarks: 50, weightage: 25, tests: ['Reading Aloud', 'Oral Interaction'] }
    }
  },
  'Higher Chinese': {
    papers: {
...
// Grade bands are the commonly used O-Level percentage cut-offs (estimates — cross-check with teachers).
const OLEVEL_GRADES = [
  { grade: 'A1', min: 75 }, { grade: 'A2', min: 70 }, { grade: 'B3', min: 65 }, { grade: 'B4', min: 60 },
  { grade: 'C5', min: 55 }, { grade: 'C6', min: 50 }, { grade: 'D7', min: 45 }, { grade: 'E8', min: 40 }, { grade: 'F9', min: 0 }
];
const GRADE_COLORS = { A1:'#10b981', A2:'#10b981', B3:'#22c55e', B4:'#84cc16', C5:'#f59e0b', C6:'#f59e0b', D7:'#f97316', E8:'#f97316', F9:'#ef4444' };

function gradeForPercentage(pct) {
  for (const g of OLEVEL_GRADES) if (pct >= g.min) return g;
  return OLEVEL_GRADES[OLEVEL_GRADES.length - 1];
}
...
function ensureBuiltinExams(activeKeys) {
  ...
  allBuiltinKeys.forEach(key => {
    ...
    if (!state.examSets[id] && BUILTIN_EXAMS[key]) {
          state.examSets[id] = { name: BUILTIN_EXAMS[key].name, type: 'builtin', builtinKey: key, enabled: false, selectedIds: [], progress: {}, notes: {}, goals: {}, topics: {}, scores: {}, subjectColors: {} };
      changed = true;
    }
  });
  if (activeKeys) {
...
function createCustomExamSet(name, papers) {
  ...
  state.examSets[id] = {
    ...
    selectedIds: papers.map((_, i) => id + '-p' + i), progress: {}, notes: {}, goals: {}, topics: {}, scores: {}, subjectColors: colors
  };
...
function buildResultsGroups() {
  const map = new Map();
  getSelectedExams().forEach(paper => {
    const meta = SEAB_SUBJECT_META[paper.subject];
    if (!meta) return;
    const pMeta = meta.papers[paper.paper] || meta.papers[paper.paperName];
    if (!pMeta) return;
    const setId = findExamSetForPaper(paper.id);
    const key = meta.group || paper.subject;
    if (!map.has(key)) map.set(key, { subjects: new Set(), papers: [], setId: null });
    const g = map.get(key);
    g.subjects.add(paper.subject);
    g.papers.push({ paper, meta: pMeta, setId });
    if (!g.setId) g.setId = setId;
  });
  return [...map.values()].map(g => {
    const label = [...g.subjects].map(s => SEAB_SUBJECT_META[s].label).find(Boolean) || [...g.subjects][0] || 'Subject';
    return { label, papers: g.papers.sort((a, b) => a.paper.date < b.paper.date ? -1 : 1), setId: g.setId };
...
function renderResults() {
  ...
  if (!container) return;
  const groups = buildResultsGroups();
  if (!groups.length) {
    container.innerHTML = `<div class="rs-empty"><div style="font-size:32px;margin-bottom:8px">📊</div><p><strong>No results data yet.</strong></p><p style="font-size:13px;margin-top:6px">Select your subjects in Settings — subjects with SEAB weightage data will appear here.</p></div>`;
    return;
  }
  container.innerHTML = groups.map(g => {
    const color = getSubjectColor(g.label, g.setId);
    const totalWeight = g.papers.reduce((s, e) => s + e.meta.weightage, 0);
    let sum = 0;
    g.papers.forEach(e => {
      const set = state.examSets[e.setId];
      const scores = (set && set.scores) || {};
      const marks = scores[e.paper.id] || 0;
      sum += e.meta.weightage * (marks / e.meta.maxMarks);
    });
    const pct = totalWeight > 0 ? (sum / totalWeight) * 100 : 0;
    const grade = gradeForPercentage(pct);
    const gCol = GRADE_COLORS[grade.grade];
    const coverage = totalWeight >= 99.5 ? '' : `<div class="rs-coverage-note">⚠️ Covers ${totalWeight.toFixed(0)}% of this subject — add the remaining papers for a full grade.</div>`;
    return `<div class="results-subject">
      <div class="results-subject-header">
        <span style="width:10px;height:10px;border-radius:50%;background:${color};display:inline-block;flex-shrink:0"></span>
        <span class="results-subject-title">${escHtml(g.label)}</span>
        <span class="rs-grade-badge" style="background:${gCol}22;color:${gCol};border:1px solid ${gCol}55">${grade.grade} (est.)</span>
      </div>
      ${g.papers.map(e => {
        const set = state.examSets[e.setId];
        const scores = (set && set.scores) || {};
        const marks = scores[e.paper.id] || 0;
        const pctP = (marks / e.meta.maxMarks) * 100;
        const contrib = e.meta.weightage * pctP / 100;
        const name = e.paper.paper + (e.paper.paperName && e.paper.paperName !== e.paper.paper ? ' — ' + e.paper.paperName : '');
        return `<div class="rs-paper">
          <div class="rs-paper-title"><span>${escHtml(name)}</span><span class="rs-weight">${e.meta.weightage}% of subject</span></div>
          <div class="rs-tests">${e.meta.tests.map(t => `<span>${escHtml(t)}</span>`).join('')}</div>
          <div class="rs-slider-row">
            <input type="range" class="rs-slider" min="0" max="${e.meta.maxMarks}" step="1" value="${marks}" data-set="${e.setId}" data-paper="${e.paper.id}" data-max="${e.meta.maxMarks}" data-weight="${e.meta.weightage}" aria-label="${escHtml(name)} marks">
            <span class="rs-marks">${marks} / ${e.meta.maxMarks} marks</span>
            <span class="rs-paper-pct">${pctP.toFixed(1)}%</span>
          </div>
          <div class="rs-contrib">contributes ${contrib.toFixed(2)} of the ${e.meta.weightage}% weightage</div>
        </div>`;
      }).join('')}
      <div class="rs-subject-summary">
        <span class="rs-overall-label">Overall</span>
        <span class="rs-overall-pct">${pct.toFixed(3)}%</span>
        ${coverage}
      </div>
    </div>`;
  }).join('');
...
function wireResultsSliders() {
  document.querySelectorAll('#results-content .rs-slider').forEach(slider => {
    slider.addEventListener('input', () => {
      slider.closest('.rs-paper').querySelector('.rs-marks').textContent = `${slider.value} / ${slider.dataset.max} marks`;
      updateResultsCard(slider.closest('.results-subject'));
    });
    slider.addEventListener('change', () => {
      const set = state.examSets[slider.dataset.set];
      if (set) {
        if (!set.scores) set.scores = {};
        set.scores[slider.dataset.paper] = parseInt(slider.value);
        saveState();
      }
      updateResultsCard(slider.closest('.results-subject'));
    });
...
function updateResultsCard(card) {
  if (!card) return;
  let sum = 0, totalWeight = 0;
  card.querySelectorAll('.rs-slider').forEach(slider => {
    const max = parseInt(slider.dataset.max);
    const w = parseFloat(slider.dataset.weight);
    const marks = parseInt(slider.value) || 0;
    const pctP = max > 0 ? (marks / max) * 100 : 0;
    const contrib = w * pctP / 100;
    totalWeight += w;
    sum += w * (marks / max);
    const row = slider.closest('.rs-paper');
    if (row) {
      row.querySelector('.rs-paper-pct').textContent = pctP.toFixed(1) + '%';
      row.querySelector('.rs-contrib').textContent = `contributes ${contrib.toFixed(2)} of the ${w}% weightage`;
    }
  });
  const pct = totalWeight > 0 ? (sum / totalWeight) * 100 : 0;
  const grade = gradeForPercentage(pct);
  const gCol = GRADE_COLORS[grade.grade];
  const pctEl = card.querySelector('.rs-overall-pct');
  if (pctEl) pctEl.textContent = pct.toFixed(3) + '%';
  const badge = card.querySelector('.results-subject-header .rs-grade-badge');
  if (badge) {
    badge.textContent = grade.grade + ' (est.)';
    badge.style.background = gCol + '22';
    badge.style.color = gCol;
    badge.style.borderColor = gCol + '55';
  }
  const note = card.querySelector('.rs-coverage-note');
  if (note) note.style.display = totalWeight >= 99.5 ? 'none' : '';
}
...
function exportData() {
  ...
  Object.entries(state.examSets).forEach(([id, set]) => {
    data.examSets[id] = {
      ...
      selectedIds: set.selectedIds, progress: set.progress, notes: set.notes, scores: set.scores,
  ...
  if (state.firstVisit) {
    ...
    document.getElementById('setup-save').addEventListener('click', () => {
      ...
      if (!checked.length) {
        ...
        state.examSets['custom-default'] = { name: 'My Exams', type: 'custom', enabled: true, papers: [], selectedIds: [], progress: {}, notes: {}, goals: {}, topics: {}, scores: {}, subjectColors: {} };
      } else {
