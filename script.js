...
function renderList() {
  ...
  const view = document.querySelector('.view-tab.active')?.dataset?.view || 'all';
  let filtered = exams;
...
function exportData() {
  ...
  Object.entries(state.examSets).forEach(([id, set]) => {
    data.examSets[id] = {
      ...
      resultScores: set.resultScores, resultWeights: set.resultWeights, resultTargets: set.resultTargets,
...
(async () => {
  ...
  // Results tab — score/weight inputs and target grade selects
  ...
  resultsView.addEventListener('change', (e) => {
    ...
    const set = state.examSets[t.dataset.set]; if (!set) return;
    if (!set.resultTargets) set.resultTargets = {};
  ...
  document.getElementById('btn-list-all').addEventListener('click', () => {
    const cur = document.querySelector('.view-tab.active')?.dataset.view || 'all';
