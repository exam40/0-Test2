...
function pickPlotPapers(pool, weights) {
  const sorted = [...pool].sort((a, b) => ((weights[b.id] || 0) - (weights[a.id] || 0)) || a.paper.localeCompare(b.paper));
  return { p1: sorted[0], p2: sorted[1] };
}
function clipHalfPlane(w1, w2, C) {
  const sq = [[0, 0], [100, 0], [100, 100], [0, 100]];
  const eps = 1e-6;
  const out = [];
  for (let i = 0; i < sq.length; i++) {
    const cur = sq[i], nxt = sq[(i + 1) % sq.length];
    const curIn = w1 * cur[0] + w2 * cur[1] >= C - eps;
    const nxtIn = w1 * nxt[0] + w2 * nxt[1] >= C - eps;
    if (curIn) out.push(cur);
    if (curIn !== nxtIn) {
      const den = w1 * (nxt[0] - cur[0]) + w2 * (nxt[1] - cur[1]);
      if (Math.abs(den) > eps) {
        const t = (C - (w1 * cur[0] + w2 * cur[1])) / den;
...
function resultsStatusHtml(r) {
  let main;
  if (r.knownW === 0) main = '<div class="results-status muted">Enter scores for some papers to see what you need for your target grade.</div>';
  else if (r.remW <= 0) {
    const ok = r.currentPct >= r.minPct;
    main = `<div class="results-status ${ok ? 'ok' : 'bad'}">All papers entered &mdash; weighted score <strong>${r.currentPct.toFixed(1)}%</strong>. ${ok ? 'On track for <strong>' + r.target + '</strong>.' : 'Below <strong>' + r.target + '</strong> (' + r.minPct + '% needed).'}</div>`;
  } else if (!r.achievable) main = `<div class="results-status bad">Target <strong>${r.target}</strong> (${r.minPct}%) is <strong>not achievable</strong> &mdash; even 100% on the remaining paper${r.remCount > 1 ? 's' : ''} only reaches <strong>${Math.min(100, ((r.knownPts + r.remW * 100) / r.totalW)).toFixed(1)}%</strong>.</div>`;
  else if (r.needRel <= 0) main = `<div class="results-status ok">Already on track for <strong>${r.target}</strong> (${r.minPct}%). Keep it up!</div>`;
  else main = `<div class="results-status need">To hit <strong>${r.target}</strong> (${r.minPct}%), the remaining paper${r.remCount > 1 ? 's' : ''} must average about <strong>${Math.min(100, r.needRel).toFixed(1)}%</strong>.</div>`;
  const proj = r.knownW > 0 && r.remW > 0 ? `<div class="results-projected">Projected ~<strong>${r.projectedPct.toFixed(1)}%</strong> so far &middot; likely grade <strong>${gradeBand(r.scale, r.projectedPct).join('&ndash;')}</strong> (estimate)</div>` : '';
  return main + proj;
}
function combinationExamples(r) {
  if (r.remCount === 0 || r.remCount < 2 || r.needRel <= 0 || !r.achievable) return [];
  const remain = r.papers.filter(p => scoreOf(r, p.id) === null);
  const wTotal = remain.reduce((a, p) => a + (r.weights[p.id] || 0), 0);
  const required = Math.max(0, r.minPct * r.totalW - r.knownPts);
  if (wTotal <= 0) return [];
  const combos = [];
  const eq = required / wTotal;
  if (eq <= 100) combos.push({ label: 'Balanced', values: remain.map(p => ({ paper: p, pct: eq })) });
  const strongest = remain.reduce((a, b) => (r.weights[b.id] || 0) > (r.weights[a.id] || 0) ? b : a);
  const wStrong = r.weights[strongest.id] || 0;
  const boost = Math.min(100, eq + 15);
  const restNeed = (wTotal - wStrong) > 0 ? (required - boost * wStrong) / (wTotal - wStrong) : 0;
  if (restNeed >= 0 && restNeed <= 100) combos.push({ label: 'Lean on ' + strongest.paper, values: remain.map(p => ({ paper: p, pct: p.id === strongest.id ? boost : restNeed })) });
  const weakest = remain.reduce((a, b) => (r.weights[b.id] || 0) < (r.weights[a.id] || 0) ? b : a);
  const wWeak = r.weights[weakest.id] || 0;
  const drop = Math.max(0, eq - 15);
  const restNeed2 = (wTotal - wWeak) > 0 ? (required - drop * wWeak) / (wTotal - wWeak) : 0;
  if (restNeed2 >= 0 && restNeed2 <= 100) combos.push({ label: 'Keep ' + weakest.paper + ' lighter', values: remain.map(p => ({ paper: p, pct: p.id === weakest.id ? drop : restNeed2 })) });
  return combos;
}
function combosHtml(r) {
  const combos = combinationExamples(r);
  if (!combos.length) return '';
  return `<div class="results-combos-title">Example valid score combinations</div>${combos.map(c => `<div class="results-combo"><span class="results-combo-label">${c.label}</span><span class="results-combo-values">${c.values.map(v => `${v.paper.paper} <strong>${Math.round(v.pct)}%</strong>`).join(' &middot; ')}</span></div>`).join('')}<div class="results-combos-hint">Examples only &mdash; any combination that averages the required % works.</div>`;
}
function mathHtml(r) {
  ...
  let explain;
  if (r.remW <= 0) explain = r.currentPct >= r.minPct ? `Your final weighted score is <strong>${fmt(r.currentPct)}%</strong>, which meets the ${r.minPct}% needed for <strong>${r.target}</strong>.` : `Your final weighted score is <strong>${fmt(r.currentPct)}%</strong>, below the ${r.minPct}% needed for <strong>${r.target}</strong>.`;
...
function graphGrid() {
  let g = '';
  [25, 50, 75].forEach(v => { g += `<line class="g-grid" x1="${v}" y1="0" x2="${v}" y2="100"/><line class="g-grid" x1="0" y1="${v}" x2="100" y2="${v}"/>`; });
  return g;
}
function meterHtml(r, paper) {
  const cur = r.knownW > 0 ? r.currentPct : null;
  const need = r.remCount === 1 ? Math.max(0, Math.min(100, r.needRel)) : r.minPct;
  return `<div class="g-equation">Need at least <strong>${Math.round(need)}%</strong> on <strong>${paper.paper}</strong> for ${r.target} (grade boundaries are estimates).</div>
    <div class="results-meter-track"><div class="results-meter-need" style="width:${Math.min(100, need)}%"></div>${cur !== null ? `<div class="results-meter-marker" style="left:${Math.min(100, cur)}%"></div>` : ''}</div>
    <div class="results-meter-axis"><span>0%</span><span>${paper.paper} score</span><span>100%</span></div>`;
}
function graphBodyHtml(r, setId, subject) {
  const blank = r.papers.filter(p => scoreOf(r, p.id) === null);
  if (r.remCount === 0) return '';
  if (r.papers.length < 2 || r.remCount === 1) return meterHtml(r, blank[0]);
  const { p1, p2 } = pickPlotPapers(blank, r.weights);
  const w1 = r.weights[p1.id] || 0, w2 = r.weights[p2.id] || 0;
  const others = r.papers.filter(p => p.id !== p1.id && p.id !== p2.id);
  let K = 0, W_other = 0;
  others.forEach(p => { const w = r.weights[p.id] || 0; const s = scoreOf(r, p.id); if (s !== null) K += w * s; else W_other += w; });
  const C = Math.max(0, r.minPct * r.totalW - K - W_other * 100);
  const impossible = C > w1 * 100 + w2 * 100 + 1e-6;
  const poly = impossible ? [] : clipHalfPlane(w1, w2, C);
  const bal = (w1 + w2) > 0 ? Math.max(0, Math.min(100, C / (w1 + w2))) : 0;
  const region = poly.length ? `<polygon class="g-region" points="${poly.map(p => p.join(',')).join(' ')}"/>` : '';
  const cross = `<line class="g-cross" x1="${bal}" y1="0" x2="${bal}" y2="100"/><line class="g-cross" x1="0" y1="${bal}" x2="100" y2="${bal}"/>`;
  let note = '';
  if (impossible) note = '<div class="g-note g-note-bad">This target is not reachable regardless of these two paper scores.</div>';
  else if (C <= 0) note = '<div class="g-note">Target is already achievable at any scores on these two papers.</div>';
  return `<svg class="results-graph" viewBox="0 0 100 100" preserveAspectRatio="none" data-set="${setId}" data-subject="${subject}" data-p1="${p1.id}" data-p2="${p2.id}">
    <rect class="g-bg" x="0" y="0" width="100" height="100"/>
    ${graphGrid()}
    ${region}
    ${cross}
    <circle class="g-marker" cx="${bal}" cy="${bal}" r="3.2"/>
  </svg>
  <div class="g-axes"><span>${p1.paper} &rarr;</span><span>${p2.paper} &darr;</span></div>
  <div class="g-equation">You still need <strong>${fmt(Math.max(0, C))}</strong> weighted points from these two papers for <strong>${r.target}</strong> (${r.minPct}%). Any score pair inside the shaded zone works.</div>
  ${note}
  <div class="g-hint">Drag the dot to try different score combinations.</div>`;
}
function renderResultsSubject(setId, subject) {
  ...
  const rows = r.papers.map(p => {
    ...
    const contrib = s !== null ? `<span class="results-contrib">= ${(w * s / 100).toFixed(1)}% of final</span>` : '';
    return `<div class="results-row"><span class="results-paper">${p.paper}${p.paperName && p.paperName !== p.paper ? ' <span class="results-paper-sub">' + p.paperName + '</span>' : ''}</span><input type="number" min="0" max="100" step="0.5" class="results-weight-input" data-set="${setId}" data-paper="${p.id}" value="${Number(w).toFixed(w % 1 ? 1 : 0)}" title="Paper weighting (%)" aria-label="Weighting for ${p.paper}"><input type="number" min="0" max="100" step="0.5" class="results-score-input" data-set="${setId}" data-paper="${p.id}" placeholder="Score %" value="${s !== null ? s : ''}" aria-label="Score for ${p.paper}">${contrib}</div>`;
  }).join('');
  const sid = setId + '-' + subject.replace(/[^a-z0-9]/gi, '_');
  return `<div class="results-card" style="border-left-color:${color}"><div class="results-card-head"><div class="results-subject" style="color:${color}">${subject}</div><label class="results-target"><span>Target</span><select class="results-target-select" data-set="${setId}" data-subject="${subject}">${targetOptions}</select></label></div><div class="results-rows">${rows}</div><div id="results-status-${sid}" class="results-status-wrap">${resultsStatusHtml(r)}</div><div id="results-combos-${sid}" class="results-combos-wrap">${combosHtml(r)}</div><div id="results-graph-${sid}" class="results-graph-wrap">${graphBodyHtml(r, setId, subject)}</div>${simulatorHtml(r, setId, subject)}${mathHtml(r)}<div class="results-note">Weights come from syllabus defaults where known &mdash; edit to match yours. Grade boundaries shown are <strong>estimates</strong>.</div></div>`;
}
...
function updateResultsCard(setId, subject) {
  const r = computeSubjectResult(setId, subject);
  const sid = setId + '-' + subject.replace(/[^a-z0-9]/gi, '_');
  const st = document.getElementById('results-status-' + sid);
  if (st) st.innerHTML = resultsStatusHtml(r);
  const cb = document.getElementById('results-combos-' + sid);
  if (cb) cb.innerHTML = combosHtml(r);
  const gr = document.getElementById('results-graph-' + sid);
  if (gr) gr.innerHTML = graphBodyHtml(r, setId, subject);
  const card = st ? st.closest('.results-card') : null;
  if (!card) return;
...
// Results view inputs
  ...
  const setGraphPoint = (g, e) => {
    const rect = g.getBoundingClientRect();
    if (!rect.width) return;
    const x = Math.max(0, Math.min(100, ((e.clientX - rect.left) / rect.width) * 100));
    const y = Math.max(0, Math.min(100, ((e.clientY - rect.top) / rect.height) * 100));
    const setId = g.dataset.set, p1 = g.dataset.p1, p2 = g.dataset.p2, subject = g.dataset.subject;
    const set = state.examSets[setId]; if (!set) return;
    set.resultScores[p1] = Math.round(x * 10) / 10;
    set.resultScores[p2] = Math.round(y * 10) / 10;
    const mk = g.querySelector('.g-marker');
    if (mk) { mk.setAttribute('cx', x); mk.setAttribute('cy', y); }
    g.querySelectorAll('.g-cross').forEach((l, i) => { if (i === 0) l.setAttribute('x1', x); if (i === 0) l.setAttribute('x2', x); if (i === 1) l.setAttribute('y1', y); if (i === 1) l.setAttribute('y2', y); });
    [['p1', x], ['p2', y]].forEach(([role, val]) => {
      const inp = document.querySelector(`input.results-score-input[data-set="${setId}"][data-paper="${g.dataset[role]}"]`);
      if (inp) inp.value = val.toFixed(1);
    });
    updateResultsStatus(setId, subject);
  };
  resultsView.addEventListener('pointerdown', (e) => {
    const g = e.target.closest('.results-graph');
    if (!g) return;
    dragGraph = g;
  ...
  resultsView.addEventListener('pointermove', (e) => {
    if (dragGraph) setGraphPoint(dragGraph, e);
  });
  resultsView.addEventListener('pointerup', (e) => {
    if (dragGraph) {
      const g = dragGraph;
      dragGraph = null;
      try { if (g.hasPointerCapture(e.pointerId)) g.releasePointerCapture(e.pointerId); } catch {}
      saveState();
      updateResultsCard(g.dataset.set, g.dataset.subject);
    }
  });
  resultsView.addEventListener('pointercancel', (e) => {
    dragGraph = null;
    try { if (e.target.hasPointerCapture && e.target.hasPointerCapture(e.pointerId)) e.target.releasePointerCapture(e.pointerId); } catch {}
  });
...
function showInstallBanner(show) {
  ...
  if (!b) { b = document.createElement('div'); b.id = 'install-banner'; b.className = 'hidden'; b.innerHTML = '<span>Install this app for offline use and quick access.</span><button class="btn btn-primary" style="margin-left:8px">Install</button>'; document.body.appendChild(b); b.querySelector('button').addEventListener('click', async () => { const prompt = window.__deferredInstall; if (prompt) { prompt.prompt(); await prompt.userChoice; window.__deferredInstall = null; showInstallBanner(false); } }); }
